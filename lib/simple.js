/*
* Last update : January  January 15, January 15, 2026: January 15, 2026, 11:January 15, 2026, 1117 AM WIB 
*/
...
conn.isLid = {
    ...
    get(key) {
        return isLidCache.get(key);
    },

    has(key) {
        return isLidCache.has(key);
    },
  ...
  conn.delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms))
    ...
    conn.getFile = async (PATH, returnAsFilename) => {
     ...
     conn.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
        if (caption && typeof caption === 'string' && !options._noWm) {
...
`.trim()
    ...
    conn.reply = (jid, text = '', quoted, options) => {
        if (typeof text === 'string' && text.length > 30 && !options?._noWm) {
            let _skip = [global.wait, global.eror, global.stiker_wait]
            if (!_skip.some(s => text.includes(s))) {
                text = text.trimEnd() + '\n\n_' + global.wm + '_'
            }
        }
...
exports.smsg = (conn, m, hasParent) => {
    ...
    if (m.key) {
        ...
        if (chatJid?.endsWith('@lid')) {
            if (!m.isGroup && (m.key.senderPn || m.sender?.endsWith('@s.whatsapp.net'))) {
                chatJid = m.key.senderPn || m.sender;
            } else {
