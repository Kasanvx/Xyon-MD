	...
	if (m.id.startsWith('3EB0') || (m.id.startsWith('BAE5') && m.id.length === 16 || m.isBaileys && m.fromMe)) return;	
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (global.gcId && !m.fromMe) {
                        try {
                            const gcMeta = await conn.groupMetadata(global.gcId).catch(() => null)
                            if (gcMeta) {
                                const gcMembers = gcMeta.participants.map(p => (conn.getJid(p.id) || p.id).replace(/[^0-9]/g, ''))
                    ...
                    try {
                        ...
                        if (e) {
                            ...
                            let apiKeysInfo = typeof APIKeys !== 'undefined' ? APIKeys : (global.APIKeys || {})
                            for (let key of Object.values(apiKeysInfo)) {
                                if (key && typeof key === 'string' && key.trim().length > 0) {
                                    text = text.replace(new RegExp(key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), '#HIDDEN#')
                                }
                            }
                            if (e.name)
